/**
 * Approve Join Request API Route
 * @description Proxies join request approval to backend
 */
...
export async function POST(
  ...
  console.log(`✅ POST /api/organizations/${orgId}/join-requests/${requestId}/approve - Approving request`);

  try {
    const body = await request.json().catch(() => ({}));

    const backend_response = await fetch(
      `${BACKEND_URL}/api/organizations/${orgId}/join-requests/${requestId}/approve`,
      {
        method: 'POST',
