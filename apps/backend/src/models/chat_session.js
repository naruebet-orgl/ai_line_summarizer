...
ChatSessionSchema.index({ organization_id: 1, status: 1 });
ChatSessionSchema.index({ organization_id: 1, start_time: -1 });
...
/**
 * Create new session with organization context
 ...
 * @param {string} roomType - Room type (individual/group)
 * @returns {Promise<ChatSession>} Created session
 */
ChatSessionSchema.statics.create_session_with_org = async function(organizationId, roomId, ownerId, lineRoomId, roomName, roomType) {
  console.log(`🆕 Creating new chat session with org for room: ${roomId}`);
  ...
  if (existingSession) {
    ...
    if (!existingSession.organization_id && organizationId) {
      existingSession.organization_id = organizationId;
 ...
 */
ChatSessionSchema.statics.get_sessions_by_organization = function(organizationId, options = {}) {
  console.log(`🏢 Getting sessions for organization: ${organizationId}`);
  const {
    status = null,
    limit = 50,
    skip = 0,
  ...
  return this.find(filter)
    ...
    .populate('owner_id', 'name email')
    .populate('summary_id', 'content key_topics')
    .select('session_id room_id owner_id line_room_id status start_time end_time room_name room_type organization_id')
    .sort({ start_time: -1 })
    .limit(limit)
    .skip(skip);
};
...
/**
 * Get active sessions by organization
 * @param {ObjectId} organizationId - Organization ID
 * @returns {Promise<Array>} Array of active sessions
 */
ChatSessionSchema.statics.get_active_sessions_by_organization = function(organizationId) {
  console.log(`🏢 Getting active sessions for organization: ${organizationId}`);
  return this.find({
    organization_id: organizationId,
    status: 'active'
  })
 ...
 */
ChatSessionSchema.statics.count_by_organization = function(organizationId, status = null) {
  const filter = { organization_id: organizationId };
  if (status) filter.status = status;
 ...
 */
ChatSessionSchema.statics.get_organization_session_stats = function(organizationId) {
  console.log(`📊 Getting organization session stats: ${organizationId}`);
