...
const MessageSchema = new Schema({
  ...
  organization_id: {
    ...
    index: true,
    description: 'Organization this message belongs to'
  },
  session_id: {
    type: String,
    ref: 'ChatSession',
...
MessageSchema.index({ organization_id: 1, timestamp: -1 }); // Organization messages by time
MessageSchema.index({ organization_id: 1, room_id: 1 }); // Organization + room
 ...
 */
MessageSchema.statics.get_messages_by_organization = function(organizationId, options = {}) {
  console.log(`🏢 Getting messages for organization: ${organizationId}`);
  const {
    limit = 100,
    skip = 0,
    roomId = null,
 ...
 */
MessageSchema.statics.count_by_organization = function(organizationId) {
  return this.countDocuments({ organization_id: organizationId });
};

/**
 * Get organization message statistics
 * @param {ObjectId} organizationId - Organization ID
 * @returns {Promise<Object>} Message statistics
 */
MessageSchema.statics.get_organization_message_stats = function(organizationId) {
  console.log(`📊 Getting organization message stats: ${organizationId}`);
...
/**
 * Get messages in organization's room
 * @param {ObjectId} organizationId - Organization ID
 ...
 * @returns {Promise<Array>} Array of messages
 */
MessageSchema.statics.get_organization_room_messages = function(organizationId, roomId, limit = 50, skip = 0) {
  return this.find({
